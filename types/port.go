...
func NewPort(c interface{}, t Type, broadcast bool) (Port, error) {
	...
	go func() {
      ...
      // Build final select case list
			selCases := []reflect.SelectCase{stopCase, setCase, internalRecvCase, 
        patchRecvCase, internalSendCase, patchSendCase}
      ...
      // Handle output
            ...
            // If patch is nil never send data and listen to nil
						patch = newPatch
						patchSend = neverListen
						patchRecv = newPatch
						setPatchChan <- nil
...
func (ip InputPort) BuildInput(c chan<- Object, t Type, broadcast bool) {
  // Error is ignored because it is only returned for values of c that aren't chans
...
func (op OutputPort) BuildOutput(c <-chan Object, t Type, broadcast bool) {
  port, _ := NewPort(c, t, broadcast)
  op = OutputPort(port)
}

func (op OutputPort) Output() Port {
  return Port(op)
}
